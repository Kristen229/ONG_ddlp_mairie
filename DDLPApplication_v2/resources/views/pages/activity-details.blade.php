...
<div class="bg-gray-50 min-h-screen">
    <section class="bg-white border-b border-gray-100">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <a href="{{ route('accueil') }}#activite" class="inline-flex items-center gap-2 text-sm font-bold text-blue-700 hover:text-blue-800 mb-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div>
                        <p class="text-sm font-black uppercase tracking-wider text-blue-700 mb-2">{{ $activity->created_at->format('d/m/Y') }}</p>
                        <h1 class="text-3xl lg:text-4xl font-black text-gray-900 leading-tight">{{ $activity->titre }}</h1>
                    </div>

                    <a href="{{ route('association.details', $activity->user->id) }}" class="flex items-center gap-4 rounded-2xl bg-gray-50 border border-gray-100 p-4 hover:bg-blue-50 hover:border-blue-100 transition-colors">
                        <div class="h-14 w-14 rounded-xl bg-white overflow-hidden border border-gray-100 shadow-sm shrink-0">
                            ...
                            @else
                                <div class="w-full h-full flex items-center justify-center bg-blue-50 text-blue-700 font-black">{{ substr($activity->user->name, 0, 1) }}</div>
                            @endif
                        </div>
                        <div>
