...
<body class="bg-gray-100 text-gray-900 flex min-h-screen overflow-hidden" x-data="{ sidebarOpen: true }">
    ...
    <aside :class="sidebarOpen ? 'w-64' : 'w-20'" class="bg-slate-900 flex-shrink-0 transition-all duration-300 ease-in-out flex flex-col shadow-2xl relative z-20 h-screen hidden md:flex">
        ...
        <nav class="flex-grow py-6 px-3 space-y-2 overflow-y-auto">
            ...
            <a href="{{ route('admin.requests.index') }}" class="flex items-center px-3 py-3 rounded-xl transition-colors relative {{ request()->routeIs('admin.requests.*') ? 'bg-teal-600 text-white shadow-md' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}" title="Courriers & Demandes">
            ...
            <a href="{{ route('admin.activities.index') }}" class="flex items-center px-3 py-3 rounded-xl transition-colors relative {{ request()->routeIs('admin.activities.*') ? 'bg-teal-600 text-white shadow-md' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}" title="Activités">
            ...
            @if(auth('admin')->check() && auth('admin')->user()->is_super_admin)
	            <a href="{{ route('admin.superadmin.index') }}" class="flex items-center px-3 py-3 rounded-xl transition-colors relative {{ request()->routeIs('admin.superadmin.*') ? 'bg-teal-600 text-white shadow-md' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}" title="Gestion Admins">
	            ...
	            <a href="{{ route('admin.audit-logs.index') }}" class="flex items-center px-3 py-3 rounded-xl transition-colors {{ request()->routeIs('admin.audit-logs.*') ? 'bg-teal-600 text-white shadow-md' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}" title="Logs d'audit">
        ...
        <header class="h-20 bg-white border-b border-gray-200 flex items-center justify-between px-6 lg:px-10 z-10">
            ...
            <div class="flex items-center gap-6">
                <div x-data="{ notificationsOpen: false }" class="relative">
                    <button @click="notificationsOpen = !notificationsOpen" @click.away="notificationsOpen = false" class="relative p-2 text-gray-400 hover:text-teal-600 transition-colors">
                    ...
                    <div x-show="notificationsOpen" x-transition class="absolute right-0 mt-3 w-80 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                        <div class="px-4 py-3 border-b border-gray-100">
                            <p class="text-sm font-black text-slate-900">Actions en attente</p>
                            <p class="text-xs text-slate-500">{{ $totalPendingAdmin }} element(s) a traiter</p>
                        </div>
                        <div class="divide-y divide-gray-100">
                            <a href="{{ route('admin.candidates.index') }}" class="flex items-center justify-between px-4 py-3 hover:bg-slate-50 transition-colors">
                                ...
                                <span class="text-xs font-black px-2 py-1 rounded-full {{ $pendingCandidatesCount > 0 ? 'bg-red-50 text-red-700' : 'bg-slate-100 text-slate-500' }}">{{ $pendingCandidatesCount }}</span>
                            </a>
                            <a href="{{ route('admin.requests.index') }}" class="flex items-center justify-between px-4 py-3 hover:bg-slate-50 transition-colors">
                                ...
                                <span class="text-xs font-black px-2 py-1 rounded-full {{ $pendingRequestsCount > 0 ? 'bg-red-50 text-red-700' : 'bg-slate-100 text-slate-500' }}">{{ $pendingRequestsCount }}</span>
                            </a>
                            <a href="{{ route('admin.activities.index') }}" class="flex items-center justify-between px-4 py-3 hover:bg-slate-50 transition-colors">
                                <span class="text-sm font-bold text-slate-700">Activites a valider</span>
                                <span class="text-xs font-black px-2 py-1 rounded-full {{ $pendingActivitiesCount > 0 ? 'bg-red-50 text-red-700' : 'bg-slate-100 text-slate-500' }}">{{ $pendingActivitiesCount }}</span>
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div x-data="{ userMenu: false }" class="relative">
                    <button @click="userMenu = !userMenu" @click.away="userMenu = false" class="flex items-center gap-3 focus:outline-none bg-gray-50 p-2 rounded-xl border border-gray-100 hover:bg-gray-100 transition-colors">
                        ...
                        <span class="text-sm font-bold text-gray-700 hidden md:block">{{ auth('admin')->user()->prenom ?: 'Admin' }}</span>
