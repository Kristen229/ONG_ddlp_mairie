            ...
            <form method="POST" action="{{ route('admin.update', $user->id) }}" class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="name" value="{{ old('name', $user->name) }}" required class="block w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 text-base">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="denomination" value="{{ old('denomination', $user->denomination) }}" required class="block w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 text-base">
                    </div>
                    <div class="md:col-span-2">
                    ...
                    <div class="md:col-span-2" x-show="Array.isArray(domaine) ? domaine.includes('Autre') : false" x-transition>
                        ...
                        <input type="text" name="domaine_autre" value="{{ old('domaine_autre', $hasAutre ? collect($userDomaines)->reject(fn($d) => in_array($d, $domaines) || $d === 'Autre')->first() : '') }}" class="block w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 text-base" :required="Array.isArray(domaine) ? domaine.includes('Autre') : false">
                    </div>
                    <div>
                        <label class="block text-base font-bold text-gray-700 mb-1">Date de création légale <span class="text-red-500">*</span></label>
                        <input type="date" name="date" value="{{ old('date', $user->date ? \Carbon\Carbon::parse($user->date)->format('Y-m-d') : '') }}" required class="block w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 text-base">
                    </div>
                    <div>
                        <label class="block text-base font-bold text-gray-700 mb-1">Site Web / Réseaux sociaux</label>
                        <input type="url" name="lien" value="{{ old('lien', $user->lien) }}" placeholder="https://..." class="block w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 text-base">
                    </div>
                </div>
                ...
                <div class="flex justify-end pt-6 border-t border-gray-100">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-xl transition-colors shadow-md">Enregistrer les modifications</button>
                </div>
            </form>
        </div>
    </div>
