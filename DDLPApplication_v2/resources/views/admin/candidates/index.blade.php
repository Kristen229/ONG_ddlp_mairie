                            ...
                            <tr class="hover:bg-slate-50/70 transition-colors">
                                <td class="px-6 py-5">
                                    <div class="flex items-center gap-4">
                                        <div class="h-12 w-12 rounded-xl bg-blue-50 text-blue-700 flex items-center justify-center text-lg font-black border border-blue-100 shrink-0">
                                            {{ strtoupper(substr($candidate->name, 0, 1)) }}
                                        </div>
                                        <div class="min-w-0">
                                            <a href="{{ route('admin.candidates.show', $candidate->id) }}" class="font-black text-slate-800 hover:text-blue-700 transition-colors">
                                                {{ $candidate->name }}
                                            </a>
                                            <p class="text-xs text-slate-500 mt-1 truncate max-w-xs">{{ $candidate->denomination ?: 'Sigle non renseigné' }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-5 whitespace-nowrap">
                                    <span class="text-xs font-black uppercase tracking-wider px-2.5 py-1 rounded-lg {{ $candidate->groupe?->value === 'ong' ? 'bg-blue-50 text-blue-700 border border-blue-100' : 'bg-purple-50 text-purple-700 border border-purple-100' }}">
                                        {{ $candidate->groupe?->value ?? 'Non spécifié' }}
                                    </span>
                                </td>
                                <td class="px-6 py-5">
                                    <p class="text-sm font-medium text-slate-700 max-w-md line-clamp-2">{{ is_array($candidate->domaine) ? implode(', ', $candidate->domaine) : ($candidate->domaine ?: 'Non renseigné') }}</p>
                                </td>
                                <td class="px-6 py-5">
                                    <p class="text-sm font-bold text-slate-800">{{ $candidate->email }}</p>
                                    <p class="text-xs text-slate-500 mt-1">{{ $candidate->number1 ?: 'Téléphone non renseigné' }}</p>
                                </td>
                                <td class="px-6 py-5 whitespace-nowrap">
