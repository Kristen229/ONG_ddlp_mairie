                ...
                <form x-data="{ action: '' }" method="POST" :action="action === 'approve' ? '{{ route('admin.approve', $assocRequest->id) }}' : '{{ route('admin.reject', $assocRequest->id) }}'" enctype="multipart/form-data" id="processForm">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <textarea name="admin_response" rows="4" required class="w-full border border-slate-300 rounded-xl px-4 py-2 focus:ring-blue-500 focus:border-blue-500 text-sm" placeholder="Rédigez la réponse qui sera envoyée à l'ONG...">{{ old('admin_response') }}</textarea>
                        </div>
